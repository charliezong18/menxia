...
3. **M3 闭环** ✅（2026-07-26）：批注串与回话展示、钦此（M2.5 已上线）、**rev 版本切换**（F4 第 1 档：PR commits 列版本 + 阅读页版本下拉，只读不比对）。落地记要：批注串按 `line ?? original_line` 挂靠当前文档、outdated 标「旧」、其他文档串收进「其他 N 串」折叠组；rev 切旧版只读（浮批钮不出、草稿高亮不画、钦此/提交禁用、顶部淡提示）。5 条 `?demo=1&auto=1` smoke 全绿。迁移已把 DOM 更新点结构性消掉——M3 新功能未再触发任何闸门指标（详见 MIGRATION-WATCH）
