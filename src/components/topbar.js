// 顶栏：面包屑 + 动作区（刷新/引用/回奏对/总批/提交/钦此）+ 新版横幅与提示条。
  ...
  return html`
        <div class="mainbar">
          ...
          <span class="actions">
            ...
            ${cur && html`<button class="btn-ghost" title="拷朱批直达链（按住 Alt 拷 GitHub 链接）" onClick=${onCopyRef}>引用此处</button>`}
            ...
            ${cur && html`<button class="btn-ghost" title=${archived ? '归档折仍可留总批（划句批注才需要活折）' : ''} onClick=${onZongpi}>总批</button>`}
            ${cur && draftCount > 0 && html`
              <button class="btn-primary btn-submit" disabled=${busy || !onHead}
                ...
                ${busy ? '呈递中…' : `提交朱批 · ${draftCount}`}
              </button>`}
            ${cur && !archived && html`<button class="btn-qinci" disabled=${busy || !onHead}
              title=${onHead ? '' : '正在读旧版——先切回 head 再钦此'} onClick=${onQinci}>钦 此</button>`}
          </span>
        </div>
        ${stale && html`
